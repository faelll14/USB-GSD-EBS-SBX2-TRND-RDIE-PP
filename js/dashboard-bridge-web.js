const DashBridge = (() => {
  ...
  async function getExamSchedule() {
    ...
    try {
      ...
      }).map((item) => Object.assign({ isDaily: false }, item));

      if (session.kelas) {
        const dailyQ = query(
          collection(db, "daily_exams"),
          where("status", "==", "published"),
          where("kelasTargets", "array-contains", session.kelas),
          limit(50)
        );
  ...
  async function attemptStartExam(examId) {
    window.location.href = "/exam?examId=" + encodeURIComponent(examId);
  }
