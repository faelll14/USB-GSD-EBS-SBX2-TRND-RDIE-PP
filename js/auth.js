...
const AuthRepository = (() => {
  ...
  async function checkPlatformAccessAllowed(role) {
    const { db, doc, getDoc } = fb();
    try {
      ...
      if (snap.exists()) {
        ...
        const operationMode = snap.data().operationMode || "ujian";
        if (operationMode === "harian" && role === "panitia") {
          throw { code: "ROLE_MODE_RESTRICTED" };
        }
      }
    } catch (e) {
      if (e && (e.code === "LOGIN_DISABLED_PLATFORM" || e.code === "ROLE_MODE_RESTRICTED")) throw e;
    }
  }
  ...
  async function login(identifierType, identifierRaw, password) {
    ...
    try {
      await checkPlatformAccessAllowed(role);
    } catch (e) {
      await fb().signOut(auth);
      throw e;
    }
