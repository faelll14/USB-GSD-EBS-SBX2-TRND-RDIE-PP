const AdminBridge = (() => {
  ...
  async function createAccountInternal(request) {
    await ensureReady();
    const { db, doc, getDoc, setDoc, collection, query, where, limit, getDocs, createAuthAccountSecondary } = fb();
  ...
  async function createAccount(args) {
    ...
    const summary = await createAccountInternal(request);
    return { uid: summary.uid };
  }

  async function createAccountsBulk(accounts) {
    let created = 0;
    let failed = 0;
    const failures = [];
    for (const row of accounts) {
      ...
      try {
        await createAccountInternal(request);
        created++;
      } catch (e) {
        failed++;
        failures.push({ nisNip: request.nisNip, reason: typeof e === "string" ? e : "UNKNOWN_ERROR" });
      }
    }
