const AdminBridge = (() => {
  ...
  function getSession() {
    ...
    return Promise.resolve({ uid: stored.uid, fullName: stored.fullName, role: stored.role, passwordIsDefault: !!stored.passwordIsDefault });
  }
  ...
  async function createAccountInternal(request, existingNisNipSet) {
    ...
    if (!["siswa", "guru", "panitia", "admin"].includes(role)) throw "INVALID_INPUT";
    if (role === "siswa" && (!request.kelas || request.noAbsen == null)) throw "INVALID_INPUT";
    if (request.noAbsen != null && request.noAbsen <= 0) throw "INVALID_INPUT";
    ...
    const defaultPassword = role === "siswa" ? AppConfig.DEFAULT_PASSWORD_SISWA : AppConfig.DEFAULT_PASSWORD_STAFF;
    ...
    const profile = {
      ...
      role,
      nisNip,
      kelas: request.kelas ? request.kelas.trim() : null,
    ...
    return { uid, fullName, role, nisNip, kelas: request.kelas || null, username: null, passwordIsDefault: true, disabled: false };
  }
  ...
  async function assignRooms(pairs) {
    ...
    for (const pair of pairs) {
      const nisNip = (pair.nisNip || "").trim();
      const ruang = (pair.ruang || "").trim();
      if (!nisNip) continue;
      try {
        const q = query(collection(db, "users"), where("nisNip", "==", nisNip), limit(1));
  ...
  function mapErr(e) {
    ...
    if (c === "unavailable" || c === "deadline-exceeded" || c === "auth/network-request-failed") return "NETWORK_ERROR";
  ...
  async function addAdditionalRole(uid, role) {
    await ensureReady();
    if (!["guru", "panitia"].includes(role)) throw "INVALID_INPUT";
    const { db, doc, getDoc, updateDoc } = fb();
    try {
      const snap = await getDoc(doc(db, "users", uid));
      if (!snap.exists()) throw "NOT_FOUND";
      const data = snap.data();
      if (data.role === role) throw "INVALID_INPUT";
      const current = Array.isArray(data.additionalRoles) ? data.additionalRoles : [];
      if (current.includes(role)) return { saved: true };
      await updateDoc(doc(db, "users", uid), { additionalRoles: current.concat([role]) });
      return { saved: true };
    } catch (e) {
      if (e === "NOT_FOUND" || e === "INVALID_INPUT") throw e;
      throw "NETWORK_ERROR";
    }
  }

  async function removeAdditionalRole(uid, role) {
    await ensureReady();
    const { db, doc, getDoc, updateDoc } = fb();
    try {
      const snap = await getDoc(doc(db, "users", uid));
      ...
      const data = snap.data();
      const current = Array.isArray(data.additionalRoles) ? data.additionalRoles : [];
      await updateDoc(doc(db, "users", uid), { additionalRoles: current.filter((r) => r !== role) });
      return { saved: true };
    } catch (e) {
  ...
  async function createExamSchedule(args) {
    await ensureReady();
    const { db, doc, setDoc, runTransaction, deleteDoc } = fb();
    const newItem = {
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        const kept = existing.filter((m) => {
          const endTime = (m.startTimestamp || 0) + (m.durationMinutes || 0) * 60000;
          const expired = endTime > 0 && (now - endTime) > sevenDaysMs;
          if (expired) expiredIds.push(m.id);
          return !expired;
        });
        kept.push(newItem);
        tx.set(docRef, { items: kept });
      });
      await setDoc(doc(db, "exam_timing", newItem.id), {
        startTimestamp: newItem.startTimestamp,
        durationMinutes: newItem.durationMinutes
      });
      for (const oldId of expiredIds) {
        deleteDoc(doc(db, "exam_timing", oldId)).catch(() => {});
      }
      return { id: newItem.id };
    } catch (e) {
      console.error("createExamSchedule", e);
      throw mapErr(e);
    }
  }

  async function updateExamSchedule(examId, args) {
    await ensureReady();
    const { db, doc, setDoc, runTransaction } = fb();
    try {
      const docRef = doc(db, "exams_schedule", "published");
      await runTransaction(db, async (tx) => {
        const snap = await tx.get(docRef);
        const existing = (snap.exists() ? snap.data().items : []) || [];
        const nextItems = existing.map((m) => (m.id === examId ? Object.assign({}, m, {
          subject: args.subject || m.subject,
          kelasTargets: args.kelasTargets || m.kelasTargets,
          date: args.date || m.date,
          startLabel: args.startLabel || m.startLabel,
          startTimestamp: args.startTimestamp || m.startTimestamp,
          durationMinutes: args.durationMinutes || m.durationMinutes
        }) : m));
        tx.set(docRef, { items: nextItems });
      });
      await setDoc(doc(db, "exam_timing", examId), {
        startTimestamp: args.startTimestamp || 0,
        durationMinutes: args.durationMinutes || 0
      }, { merge: true });
      return { id: examId };
    } catch (e) {
      console.error("updateExamSchedule", e);
      throw mapErr(e);
    }
  }
  ...
  async function deleteExamSchedule(examId) {
    await ensureReady();
    const { db, doc, runTransaction, deleteDoc } = fb();
    try {
      const docRef = doc(db, "exams_schedule", "published");
      await runTransaction(db, async (tx) => {
        const snap = await tx.get(docRef);
        const existing = (snap.exists() ? snap.data().items : []) || [];
        const nextItems = existing.filter((m) => m.id !== examId);
        tx.set(docRef, { items: nextItems });
      });
      await deleteDoc(doc(db, "exam_timing", examId)).catch(() => {});
      return { id: examId };
    } catch (e) {
      console.error("deleteExamSchedule", e);
      throw mapErr(e);
    }
  }
  ...
  async function setResultsPublished(examId, published) {
    await ensureReady();
    const { db, doc, getDoc, setDoc, collection, query, where, getDocs, updateDoc, runTransaction } = fb();
    try {
      ...
      const scheduleRef = doc(db, "exams_schedule", "published");
      await runTransaction(db, async (tx) => {
        const snap = await tx.get(scheduleRef);
        if (!snap.exists()) return;
        const items = snap.data().items || [];
        const nextItems = items.map((m) => (m.id === examId ? Object.assign({}, m, { resultsPublished: published }) : m));
        tx.update(scheduleRef, { items: nextItems });
      });
  ...
  async function getDutyPlanningData(examId) {
    ...
    try {
      ...
      siswaSnap.forEach((d) => {
        ...
        targetStudentCount++;
        if (data.noRuang) {
          rooms.add(data.noRuang);
        } else {
          unassignedStudentCount++;
        }
      });

      if (targetStudentCount === 0) throw "NO_STUDENTS_IN_TARGET";

      if (unassignedStudentCount > 0) {
        return {
          roomsNotConfigured: true,
          unassignedStudentCount,
          rooms: [],
          freePanitia: [],
          existingAssignments: {}
      ...
      const panitiaQ = query(collection(db, "users"), where("role", "in", ["panitia", "guru"]), limit(500));
      const panitiaSnap = await getDocs(panitiaQ);
      const allPanitia = [];
      panitiaSnap.forEach((d) => allPanitia.push({ uid: d.id, fullName: d.data().fullName || "", nisNip: d.data().nisNip || "" }));
      ...
      return { roomsNotConfigured: false, rooms: Array.from(rooms).sort(), freePanitia, existingAssignments };
    } catch (e) {
      if (typeof e === "string") throw e;
      console.error("getDutyPlanningData", e);
      throw mapErr(e);
    }
  }
  ...
  async function saveDutyAssignments(examId, assignments) {
    ...
    try {
      ...
      const panitiaQ = query(collection(db, "users"), where("role", "in", ["panitia", "guru"]));
      const panitiaSnap = await getDocs(panitiaQ);
      const nameByUid = {};
      ...
      console.error("saveDutyAssignments", e);
      throw mapErr(e);
    }
  }
  ...
  async function changePassword(oldPassword, newPassword) {
    ...
    SessionGuard.updateStoredSession({ passwordIsDefault: false });
    return { changed: true };
  }
  ...
  async function setAccessSetting(key, value) {
    await ensureReady();
    const { db, doc, setDoc, collection, query, where, limit, getDocs, writeBatch } = fb();
    ...
    try {
      ...
      if (key === "operationMode") {
        const q = query(collection(db, "users"), where("role", "==", "panitia"), limit(1000));
        const snap = await getDocs(q);
        const batch = writeBatch(db);
        let affected = 0;
        snap.forEach((d) => {
          const data = d.data();
          const autoDisabled = !!data.autoDisabledByMode;
          const currentlyDisabled = !!data.disabled;
          if (value === "harian") {
            if (!currentlyDisabled) {
              batch.update(doc(db, "users", d.id), { disabled: true, autoDisabledByMode: true });
              affected++;
            }
          } else if (currentlyDisabled && autoDisabled) {
            batch.update(doc(db, "users", d.id), { disabled: false, autoDisabledByMode: false });
            affected++;
          }
        });
