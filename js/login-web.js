(function () {
  ...
  async function init() {
    const existing = SessionGuard.getStoredSession();
    if (existing && existing.role) {
      routeToRoleDashboard(existing.role);
      return;
    }
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const rememberLong = document.getElementById("rememberMeCheckbox").checked;
        SessionGuard.saveSession(session, rememberLong);
        ...
        const messages = {
          ...
          INVALID_CREDENTIALS: "NIS/NIP atau kata sandi salah. Periksa kembali data yang dimasukkan.",
          ...
          ROLE_MODE_RESTRICTED: "Akun panitia tidak bisa login saat sistem dalam Mode Harian (ulangan).",
  ...
  function routeToRoleDashboard(role) {
    ...
    window.location.href = pages[role] || "/dashboard";
  }
