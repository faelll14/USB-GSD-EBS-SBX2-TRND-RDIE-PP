(async function () {
  ...
  async function loadPengaturan() {
    document.getElementById("accountInfoSub").textContent = "NIS " + session.nisNip + " \u00b7 " + (session.kelas || "-");
    try {
  ...
  let knownExamIds = null;
  async function pollForNewExams() {
    try {
      const result = await DashBridge.getExamSchedule();
      scheduleCache = result;
      const currentIds = new Set(result.items.map((x) => x.id));
      if (knownExamIds === null) {
        knownExamIds = currentIds;
        return;
      }
      const newOnes = result.items.filter((x) => x.isDaily && !knownExamIds.has(x.id));
      newOnes.forEach((x) => {
        showToast("Ulangan baru: " + x.subject + ". Yuk mulai ulangan!");
      });
      knownExamIds = currentIds;
    } catch (e) {}
  }
  pollForNewExams();
  setInterval(pollForNewExams, 60000);
