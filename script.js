(async function() {
  try {
    ...
    function _setStatus(ring,progress,badge,fill,state,msg){
      ...
      if(fill)fill.className="v2l-scan-fill"+(isErr?" error":"");
    }
    ...
    async function _livenessCheck(vid,progress,badge){
      ...
      let eyeOpen=true;
      let frames=0;
      const MAX_FRAMES=150; // ~5s timeout

      return new Promise(resolve=>{
        async function tick(){
          if(frames++>MAX_FRAMES){resolve(false);return;}
          try{
            const det=await faceapi.detectSingleFace(vid,new faceapi.TinyFaceDetectorOptions({inputSize:224,scoreThreshold:0.5})).withFaceLandmarks(true);
            if(det){
              ...
              if(eyeOpen&&ear<EAR_THRESHOLD){
                ...
                if(blinkCount>=BLINK_NEEDED){resolve(true);return;}
              }
              if(progress)progress.textContent=`👁️ Kedipkan mata ${blinkCount}/${BLINK_NEEDED} kali untuk bukti hidup...`;
            } else {
        ...
        async function tick(){
          ...
          try{
            ...
            if(det){
              ...
              if(progress)progress.textContent=`👁️ Kedipkan mata sekali untuk verifikasi liveness...`;
            } else {
              if(progress)progress.textContent="🔍 Posisikan wajah di dalam lingkaran...";
            }
          }catch(e){}
    ...
    async function _startAutoCapture(videoId,canvasId,ringId,ringProgressId,fillId,progressId,badgeId,onComplete){
      ...
      _setStatus(ring,progress,badge,fill,"idle","⏳ Memuat model pengenalan wajah...");
      _setRingProgress(ringProg,0,"#4f8ef7");
      ...
      async function loop(timestamp){
        if(!vid||!canvas||captureStarted)return;
        ...
        let det=null;
        try{
          det=await faceapi.detectSingleFace(vid,new faceapi.TinyFaceDetectorOptions({inputSize:224,scoreThreshold:0.5}));
        }catch(e){}

        if(det&&det.score>0.5){
          lostFrames=0;
          stableFrames=Math.min(stableFrames+1,STABLE_NEEDED);
          const pct=stableFrames/STABLE_NEEDED;
          ...
          if(stableFrames<STABLE_NEEDED){
            ...
            captureStarted=true;
            cancelAnimationFrame(_v2lAutoDetectRAF);
            _setStatus(ring,progress,badge,fill,"capturing","👁️ Memulai uji liveness — kedipkan mata!");

            const isLive=await _livenessCheck(vid,progress,badge);
            if(!isLive){
              _setStatus(ring,progress,badge,fill,"error","❌ Liveness gagal. Silakan kedipkan mata dengan jelas.");
          ...
          if(lostFrames>LOST_THRESHOLD){
            _setRingProgress(ringProg,pct,"#ef4444");
            _setStatus(ring,progress,badge,fill,"lost","⚠️ Wajah tidak terdeteksi — pastikan wajah terlihat jelas");
          } else {
            _setRingProgress(ringProg,pct,"#22c55e");
            _setStatus(ring,progress,badge,fill,"idle","🔍 Posisikan wajah di dalam lingkaran");
          }
    ...
    async function startVerifyCamera(){
      try{
        ...
        const w=settings.width||0,h=settings.height||0;
        if(w<320||h<240){
          const alert=document.getElementById("v2lCameraQualityAlert");
        ...
        const alert=document.getElementById("v2lCameraQualityAlert");
        if(alert){alert.style.display="block";alert.textContent="Kamera tidak dapat dibuka. Gunakan metode QR Code.";}
      }
    }
    ...
    async function _startAutoVerify(){
      ...
      async function loop(timestamp){
        if(!vid||!canvas||verifyStarted)return;
        const elapsed=timestamp-lastRafTime;
        if(elapsed<1000/TARGET_FPS){_v2lVerifyAutoRAF=requestAnimationFrame(loop);return;}
        lastRafTime=timestamp;
        ...
        if(det&&det.score>0.5){
          ...
          if(stableFrames<STABLE_NEEDED){
            ...
            verifyStarted=true;
            cancelAnimationFrame(_v2lVerifyAutoRAF);
            _setStatus(ring,progress,badge,fill,"verifying","👁️ Kedipkan mata untuk verifikasi liveness...");

            const isLive=await _livenessCheckVerify(vid,progress,badge);
            if(!isLive){
              _setStatus(ring,progress,badge,fill,"error","❌ Liveness gagal. Silakan kedipkan mata sekali.");
              await new Promise(r=>setTimeout(r,2000));
              verifyStarted=false;
              stableFrames=0;
          ...
          if(lostFrames>LOST_THRESHOLD){
            _setRingProgress(ringProg,pct,"#ef4444");
            _setStatus(ring,progress,badge,fill,"error","⚠️ Wajah tidak terdeteksi — pastikan pencahayaan cukup");
          } else {
            _setRingProgress(ringProg,pct,"#22c55e");
            _setStatus(ring,progress,badge,fill,"idle","🔍 Posisikan wajah di dalam lingkaran");
          }
