  ...
  try {
      ...
      async function loop(timestamp){
        ...
        if(det&&det.score>0.5){
          ...
          if(stableFrames<STABLE_NEEDED){
            ...
            if(fill)fill.style.width=(pct*40)+"%"; // restore after _setStatus
          } else {
            captureStarted=true;
            ...
            if(!isLive){
              _setStatus(ring,progress,badge,fill,"error","❌ Waktu habis. Silakan kedipkan mata dengan jelas.");
              if(fill){fill.style.width="0%";fill.className="v2l-scan-fill";}
              _setRingProgress(ringProg,0,"#ef4444");
              await new Promise(r=>setTimeout(r,2000));
              // Reset semua state untuk mencoba lagi
              ...
              _setRingProgress(ringProg,0,"#22c55e");
              if(fill){fill.style.width="0%";fill.className="v2l-scan-fill";}
              if(_v2lAutoDetectRAF)cancelAnimationFrame(_v2lAutoDetectRAF);
              _v2lAutoDetectRAF=requestAnimationFrame(loop);
              return;
            }
    ...
    window.proceedV2LCapture=async function(){
      ...
      if(!_captLoaded){
        ...
        if(_captProg)_captProg.textContent="❌ Model gagal dimuat. Refresh halaman.";
        if(_captBadge){_captBadge.textContent="❌ Gagal";_captBadge.style.background="rgba(239,68,68,0.25)";}
        showToast("Model AI gagal dimuat","error");
        setTimeout(()=>document.getElementById("v2lModal").classList.add("hidden"),2500);
        return;
      }
    ...
    async function startVerifyCamera(){
      ...
      if(!_vLoaded){
        ...
        if(_vProg)_vProg.textContent="❌ Model gagal dimuat. Gunakan QR Code.";
        ...
        if(det&&det.score>0.5){
          ...
          if(stableFrames<STABLE_NEEDED){
            ...
            if(!isLive){
              ...
              _setRingProgress(ringProg,0,"#22c55e");
              if(fill){fill.style.width="0%";fill.className="v2l-scan-fill";}
              if(_v2lVerifyAutoRAF)cancelAnimationFrame(_v2lVerifyAutoRAF);
