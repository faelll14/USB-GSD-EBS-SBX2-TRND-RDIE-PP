(async function() {
  try {
    ...
    async function hashPassword(pwd,nis){return await sha256(pwd+"|"+nis+"|PATLAS_SALT_14DPK_v2");}
    ...
    if(soalData.soal_enc){
        const decrypted=await decryptSoalData(soalData.soal_enc);
    ...
    const encSoal=await encryptSoalData(soalArr);
    ...
    const SOAL_ENC_SALT = "PATLAS_SOAL_KUNCI_ENCRYPT_14DPK_2025";
    async function _getSoalEncKey(){
        const keyHash = await sha256(SOAL_ENC_SALT);
        const keyBytes = new Uint8Array(32);
        for(let i=0;i<32;i++) keyBytes[i]=parseInt(keyHash.slice(i*2,i*2+2),16);
        return crypto.subtle.importKey("raw",keyBytes,{name:"AES-GCM"},false,["encrypt","decrypt"]);
    }
    async function encryptSoalData(soalArr){
        try{
            const key=await _getSoalEncKey();
            const iv=crypto.getRandomValues(new Uint8Array(12));
    ...
    async function decryptSoalData(encObj){
        try{
    ...
    try{
    const adminRole=await resolveNisRole(ADMIN_NIS);
    if(!adminRole||adminRole.role!=='admin'){
    console.warn("[PATLAS] Akun admin utama belum ada. Buat lewat admin-cli (lihat SECURITY_UPDATE/09_MIGRASI_FIREBASE_AUTH), BUKAN otomatis dari sini.");
    }
    }catch(e){}
    }
    ...
    window.openKelolaSoalModal=async function(soalDocId){
        ...
        try{
            ...
            if(_kelolaSoalData.soal_enc && !_kelolaSoalData.soal){
                const decArr=await decryptSoalData(_kelolaSoalData.soal_enc);
    ...
    window.removeSoalPhoto=async function(soalIndex){
        ...
        try{
            const reEnc=await encryptSoalData(soalArr);
    ...
    window.saveKelolaSoalPhotos=async function(){
        ...
        const reEncR=await encryptSoalData(soalArr);
