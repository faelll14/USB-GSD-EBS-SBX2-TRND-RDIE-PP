...
Sekarang proses data berikut ini:`;
  ...
  document.getElementById("btnSubmitBulk").addEventListener("click", async () => {
    ...
    try {
      const result = await AdminBridge.createAccountsBulk(pendingBulkAccounts);
      showToast(result.created + " akun berhasil dibuat" + (result.failed > 0 ? ", " + result.failed + " gagal." : "."));
