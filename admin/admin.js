(async function () {
  ...
  function showConfirm(message) {
    return new Promise((resolve) => {
      const scrim = document.getElementById("simpleConfirmScrim");
      ...
      function onScrimClick(e) { if (e.target === scrim) cleanup(false); }

      const btnYes = document.getElementById("simpleConfirmYes");
      const btnCancel = document.getElementById("simpleConfirmCancel");
  ...
  const KELAS_TINGKAT_ORDER = { X: 0, XI: 1, XII: 2 };
  const KELAS_JURUSAN_ORDER = { "": 0, IPA: 1, IPS: 2 };

  function parseKelas(kelas) {
    const raw = (kelas || "").trim();
    const match = raw.match(/^(XII|XI|X)\s*(IPA|IPS)?\s*\.?\s*(\d+)?$/i);
    if (!match) return { tingkat: raw, jurusan: "", nomor: 0, valid: false, raw };
    const tingkat = match[1].toUpperCase();
    const jurusan = (match[2] || "").toUpperCase();
    const nomor = match[3] ? parseInt(match[3], 10) : 0;
    return { tingkat, jurusan, nomor, valid: true, raw };
  }

  function compareKelasKey(kelasA, kelasB) {
    const pa = parseKelas(kelasA);
    const pb = parseKelas(kelasB);
    if (pa.valid && pb.valid) {
      const ta = KELAS_TINGKAT_ORDER[pa.tingkat] ?? 9;
      const tb = KELAS_TINGKAT_ORDER[pb.tingkat] ?? 9;
      if (ta !== tb) return ta - tb;
      const ja = KELAS_JURUSAN_ORDER[pa.jurusan] ?? 9;
      const jb = KELAS_JURUSAN_ORDER[pb.jurusan] ?? 9;
      if (ja !== jb) return ja - jb;
      return pa.nomor - pb.nomor;
    }
    if (pa.valid !== pb.valid) return pa.valid ? -1 : 1;
    return pa.raw.localeCompare(pb.raw, undefined, { numeric: true, sensitivity: "base" });
  }

  function kelasSortWeight(kelas) {
    const p = parseKelas(kelas);
    const t = KELAS_TINGKAT_ORDER[p.tingkat] ?? 9;
    const j = KELAS_JURUSAN_ORDER[p.jurusan] ?? 9;
    return t * 1000 + j * 100 + p.nomor;
  }
  ...
  document.getElementById("kelasFilterSelect").addEventListener("change", (e) => {
    activeKelasFilter = e.target.value;
    renderAccounts();
  });

  function normalizeAccounts(items) {
    return items.map((a) => {
      let role = (a.role || "").toLowerCase().trim();
      if (!role) {
        role = a.kelas ? "siswa" : "admin";
      }
  ...
  async function refreshAccounts() {
    ...
    try {
      ...
      allAccounts = normalizeAccounts((result && result.items) || []);
      populateKelasFilter();
      renderAccounts();
    } catch (e) {
      const msg = e === "PERMISSION_DENIED"
        ? "Akses ditolak oleh Firestore Rules. Periksa role admin & publish rules terbaru."
        : e === "NETWORK_ERROR" ? "Tidak ada koneksi internet." : "Gagal memuat daftar akun.";
  ...
  function compareByKelasAbsen(a, b) {
    const kc = compareKelasKey(a.kelas || "", b.kelas || "");
    if (kc !== 0) return kc;
  ...
  function populateKelasFilter() {
    const sel = document.getElementById("kelasFilterSelect");
    if (!sel) return;
    const kelasSet = new Map();
    allAccounts.forEach((a) => {
      if (a.role === "siswa" && a.kelas) kelasSet.set(a.kelas, kelasSortWeight(a.kelas));
    });
    const sortedKelas = Array.from(kelasSet.keys()).sort((x, y) => kelasSet.get(x) - kelasSet.get(y));
    const prev = activeKelasFilter;
    sel.innerHTML = '<option value="all">Semua kelas</option>' +
      sortedKelas.map((k) => `<option value="${escapeHtml(k)}">${escapeHtml(k)}</option>`).join("");
    sel.value = sortedKelas.includes(prev) ? prev : "all";
    activeKelasFilter = sel.value;
  }
  ...
  function renderAccounts() {
    ...
    if (activeKelasFilter !== "all") {
      items = items.filter((a) => a.kelas === activeKelasFilter);
    }
    if (searchTerm) {
    ...
    el.innerHTML = items.map((a) => {
      const additionalRoles = a.additionalRoles || [];
      const roleBadges = [a.role].concat(additionalRoles).map((r) => `<span class="role-badge role-${r}">${r}</span>`).join("");
      let jabatanBtn = "";
      if (a.role === "guru" && !additionalRoles.includes("panitia")) {
        jabatanBtn = `<button class="btn btn-ghost btn-tiny" data-add-role="${a.uid}" data-role-value="panitia">+ Panitia</button>`;
      } else if (a.role === "guru" && additionalRoles.includes("panitia")) {
        jabatanBtn = `<button class="btn btn-ghost btn-tiny" data-remove-role="${a.uid}" data-role-value="panitia">- Panitia</button>`;
      } else if (a.role === "panitia" && !additionalRoles.includes("guru")) {
        jabatanBtn = `<button class="btn btn-ghost btn-tiny" data-add-role="${a.uid}" data-role-value="guru">+ Guru</button>`;
      } else if (a.role === "panitia" && additionalRoles.includes("guru")) {
        jabatanBtn = `<button class="btn btn-ghost btn-tiny" data-remove-role="${a.uid}" data-role-value="guru">- Guru</button>`;
      }
      return `
    ...
    el.querySelectorAll("[data-add-role]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        btn.disabled = true;
        try {
          await AdminBridge.addAdditionalRole(btn.dataset.addRole, btn.dataset.roleValue);
          showToast("Jabatan berhasil ditambahkan.");
          refreshAccounts();
        } catch (e) {
          showToast("Gagal menambahkan jabatan.");
          btn.disabled = false;
        }
      });
    });

    el.querySelectorAll("[data-remove-role]").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const confirmed = await showConfirm("Hapus jabatan tambahan ini?");
        if (!confirmed) return;
        btn.disabled = true;
        try {
          await AdminBridge.removeAdditionalRole(btn.dataset.removeRole, btn.dataset.roleValue);
          showToast("Jabatan berhasil dihapus.");
          refreshAccounts();
        } catch (e) {
          showToast("Gagal menghapus jabatan.");
          btn.disabled = false;
        }
      });
    });
  ...
  document.getElementById("btnAddJadwal").addEventListener("click", () => {
    editingExamId = null;
    document.getElementById("jadwalModalTitle").textContent = "Tambah Jadwal Ujian";
  ...
  document.getElementById("jadwalSubmit").addEventListener("click", async () => {
    ...
    try {
      if (editingExamId) {
        await AdminBridge.updateExamSchedule(editingExamId, payload);
      ...
      errorEl.textContent = e === "PERMISSION_DENIED"
        ? "Ditolak Firestore Rules (butuh role admin)."
        : e === "NETWORK_ERROR" ? "Tidak ada koneksi internet." : "Gagal menyimpan jadwal.";
    }
    btn.disabled = false;
  });
      ...
      el.innerHTML = items.map((item) => `
        <div class="card exam-card">
          ...
          <div class="question-actions">
            <button class="btn btn-ghost" data-toggle-publish="${item.id}" data-published="${item.resultsPublished ? "1" : "0"}">${item.resultsPublished ? "Sembunyikan Nilai" : "Publikasikan Nilai"}</button>
            <button class="btn btn-ghost" data-edit-jadwal="${item.id}">Edit</button>
            <button class="btn btn-ghost" data-delete-jadwal="${item.id}" style="color:var(--danger);">Hapus</button>
          </div>
        </div>
      `).join("");

      el.querySelectorAll("[data-toggle-publish]").forEach((btn) => {
        btn.addEventListener("click", () => handlePublish(btn.dataset.togglePublish, btn.dataset.published !== "1"));
      });
      el.querySelectorAll("[data-delete-jadwal]").forEach((btn) => {
        btn.addEventListener("click", () => handleDeleteJadwal(btn.dataset.deleteJadwal));
      });
      el.querySelectorAll("[data-edit-jadwal]").forEach((btn) => {
        btn.addEventListener("click", () => openEditJadwalModal(btn.dataset.editJadwal));
      });
    } catch (e) {
  ...
  async function handleDeleteJadwal(examId) {
    const confirmed = await showConfirm("Hapus jadwal ini? Tindakan ini tidak bisa dibatalkan.");
    if (!confirmed) return;
    try {
      await AdminBridge.deleteExamSchedule(examId);
  ...
  async function openEditJadwalModal(examId) {
    try {
      const result = await AdminBridge.listExamSchedule();
      const item = ((result && result.items) || []).find((m) => m.id === examId);
      if (!item) return;
      editingExamId = examId;
      document.getElementById("jadwalModalTitle").textContent = "Edit Jadwal Ujian";
      document.getElementById("jadwalSubject").value = item.subject;
      document.getElementById("jadwalDuration").value = item.durationMinutes;
      document.getElementById("jadwalDate").value = item.date;
      const dt = new Date(item.startTimestamp || Date.now());
      let hour = dt.getHours();
      const ampm = hour >= 12 ? "PM" : "AM";
      hour = hour % 12;
      if (hour === 0) hour = 12;
      document.getElementById("jadwalHour").value = hour;
      document.getElementById("jadwalMinute").value = dt.getMinutes();
      document.getElementById("jadwalAmPm").value = ampm;
      selectedGrades = new Set(item.kelasTargets || []);
      document.querySelectorAll("[data-grade]").forEach((b) => {
        b.style.opacity = selectedGrades.has(b.dataset.grade) ? "1" : "0.55";
      });
      document.getElementById("jadwalFormError").textContent = "";
      jadwalScrim.classList.add("open");
    } catch (e) {
...
Sekarang proses data berikut ini:`;
  ...
  document.getElementById("btnSubmitRoomAssign").addEventListener("click", async () => {
    ...
    const pairs = [];
    const errors = [];
    const knownSiswaNis = new Set(allAccounts.filter((a) => a.role === "siswa").map((a) => a.nisNip));
    ...
    lines.forEach((line, idx) => {
      ...
      if (knownSiswaNis.size > 0 && !knownSiswaNis.has(parts[0])) {
        errors.push("Baris " + (idx + 1) + ": NIS " + parts[0] + " tidak terdaftar sebagai akun siswa");
        return;
      }
    ...
    try {
      ...
      if (notFound.length > 0) {
        errEl.innerHTML = '<div class="parse-error-list">' +
          `<div class="parse-error-item">${result.updated} siswa berhasil ditempatkan. NIS berikut tidak ditemukan di akun terdaftar:</div>` +
          notFound.map((nis) => `<div class="parse-error-item">${escapeHtml(nis)}</div>`).join("") +
          '</div>';
        showToast(result.updated + " berhasil, " + notFound.length + " NIS tidak ditemukan.");
      } else {
  ...
  async function loadDutyExamOptions() {
    ...
    try {
      ...
      select.dispatchEvent(new Event("patsec-refresh-options"));
    } catch (e) {
  ...
  document.getElementById("dutyExamSelect").addEventListener("change", async () => {
    ...
    try {
      ...
      if (e === "NO_STUDENTS_IN_TARGET") {
        area.innerHTML = '<div class="card"><div class="empty-state">Tidak ada siswa terdaftar pada kelas target jadwal ini.</div></div>';
      } else {
  ...
  function renderDutyPlanning(examId, data) {
    ...
    if (data.roomsNotConfigured) {
      area.innerHTML = `<div class="card"><div class="empty-state">${data.unassignedStudentCount} siswa pada kelas target jadwal ini belum memiliki ruang. Tentukan ruang siswa terlebih dahulu di bagian "Atur Ruang Siswa" sebelum menetapkan penjaga.</div></div>`;
      return;
    }
    ...
    area.innerHTML = '<div class="card">' + rooms.map((room) => {
      ...
      const isAssigned = !!currentPick;
      const optionsHtml = freePanitia
        .map((p) => `<option value="${p.uid}" ${p.uid === currentPick ? "selected" : ""}>${escapeHtml(p.fullName)} (${escapeHtml(p.nisNip || "-")})</option>`)
        .join("");
      return `
        <div class="settings-row" style="padding:10px 0;flex-wrap:wrap;gap:8px;">
          <div class="settings-row-text">
            <div class="settings-row-title">Ruang ${escapeHtml(room)}</div>
            <div class="settings-row-sub" style="font-size:12px;color:${isAssigned ? "var(--success)" : "var(--warning)"};">${isAssigned ? "Sudah ada penjaga" : "Belum ada penjaga"}</div>
          </div>
    ...
    area.querySelectorAll("[data-room]").forEach((select) => {
      select.addEventListener("change", () => {
        ...
        const badge = select.closest(".settings-row").querySelector(".settings-row-sub");
        if (badge) {
          const ok = !!currentDutyAssignments[room];
          badge.textContent = ok ? "Sudah ada penjaga" : "Belum ada penjaga";
          badge.style.color = ok ? "var(--success)" : "var(--warning)";
        }
      });
    });

    if (window.CustomSelect) CustomSelect.enhanceAll(area);
